...
accuracies = np.array([0]*32)
accuracies_args = np.array([0]*32)
number = np.array([0]*32)
...
for i, data in enumerate(pbar):
    ...
    with torch.no_grad():
        ...
        # tensor of bools if there is 3
        mask = (commands_ == 3)
        lengths = 32-mask.sum(-1)
        ...
        for j in range(batch_size):
            output = {}
            output["command_logits"], output["args_logits"] = outputs[0][j].unsqueeze(0).repeat(4,1,1), outputs[1][j].unsqueeze(0).repeat(4,1,1,1)
            output["tgt_commands"], output["tgt_args"] = commands[j].unsqueeze(0).repeat(4,1), args[j].unsqueeze(0).repeat(4,1,1)
            number[lengths[j]-1] += 1
            acc_dict = acc_func(output)
            accuracies[lengths[j]-1] += acc_dict["acc_cmd"]
            accuracies_args[lengths[j]-1] += acc_dict["acc_args"]
